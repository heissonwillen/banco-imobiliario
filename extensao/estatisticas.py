def obtem_estatisticas(resultados: list[dict]):
    ...
    quantidade_timeouts = len([res for res in resultados if res.get("timeout")])
...
def imprime_estatisticas(media_turnos: int, quantidade_timeout: int, vencedores: dict, turnos: list):
    print(f"Média de turnos por partida: {media_turnos:.2f}")
    print(f"Partidas terminadas em timeout: {quantidade_timeout}")
    print(f"Porcentagem de vitórias por jogador:")
    for k, v in vencedores.items():
        print(f" - {k}: {v:.2f}%")